...
function init() {
  fetch("http://localhost:8081/api/movies/series")
    .then(res => res.json())
    .then(data => {
      moviesData = data;
      populateYears();
  ...
  document.getElementById("search-input").addEventListener("input", async () => {
    ...
    if (searchKeyword === "") {
      ...
      try {
        const res = await fetch(`http://localhost:8081/api/movies/series/search?query=${encodeURIComponent(searchKeyword)}`);
